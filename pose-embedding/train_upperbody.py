...
def parse_args():
    parser = argparse.ArgumentParser(description='Upperbody Pose embedding model training.')
    parser.add_argument(
        '--writer',
        default='runs/upperbody-train/model', #'/data2/junhe/tumeke_repetition_dataset', # RepCount_pose hand_strain/tumeke_hand_rep
        help='Tensorboard writer logs')
    parser.add_argument(
        '--ckpt-path',
        default="./checkpoints/upperbody-model-occlusion",
    ...
    parser.add_argument(
        '--keypoint-dropout-rate', type=float, default=0.2, help='keypoint-dropout-rate for occlusion')    
    parser.add_argument(
        '--resume-ckpt',
        default= None, # './checkpoints/upperbody-model-occlusion/training_model_0_600.pth', #
        help='resume training from previous checkpoint')   
    parser.add_argument(
        '--eval-interval', type=int, default=20000, help='During one epoch, the interval of doing evaluation')
    parser.add_argument(
        '--gpu-id', type=int, default=3, help='CUDA device id')
    parser.add_argument(
    ...
    tm = time.localtime()
    args.writer = args.writer + f"-{tm.tm_mon:02d}{tm.tm_mday:02d}"
    args.writer = args.writer + f'-occlusion{args.keypoint_dropout_rate:.2f}' 

    args.ckpt_path = args.ckpt_path + f'-occlusion{args.keypoint_dropout_rate:.2f}' 
...
if __name__ == '__main__':
    ...
    t0 = time.time()
    dataset = PosePairsDataset(args.dataset, keypoint_profile_2d_name='UPPERBODY_9_2D', keypoint_dropout_rate=args.keypoint_dropout_rate)
    # val_dataset = PosePairsDataset(args.val_dataset, keypoint_profile_2d_name='UPPERBODY_9_2D', keypoint_dropout_rate=args.keypoint_dropout_rate)
