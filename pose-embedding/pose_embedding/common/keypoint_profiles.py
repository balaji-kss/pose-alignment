...
import enum
import torch
from pose_embedding.common import keypoint_utils, constants, data_utils
...
class KeypointProfile(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def __init__(self,
        ...
        self._left_shoulder_keypoint_name = to_list(
            left_shoulder_keypoint_name)
        self._right_shoulder_keypoint_name = to_list(
            right_shoulder_keypoint_name)
    ...
    def keypoint_index(self, keypoint_name, raise_error_if_not_found=False):
        ...
        if raise_error_if_not_found:
            raise ValueError('Failed to find keypoint: `%s`.' %
                             str(keypoint_name))
        return -1
...
class KeypointProfile2D(KeypointProfile):
    ...
    def __init__(self,
                 ...
                 scale_distance_reduction_fn=torch.amax,
                 scale_unit=0.5,
                 head_keypoint_name=None,
                 neck_keypoint_name=None,
                 left_shoulder_keypoint_name=None,
...
class ExtractedH36m18KeypointProfile3D(KeypointProfile3D):
    """Legacy Human3.6M 3D 18-keypoint profile."""

    def __init__(self):
        """Initializer."""
        super(ExtractedH36m18KeypointProfile3D, self).__init__(
            name='EXTRACTED_3DH36M18',          
            keypoint_names=[('Pelvis', LeftRightType.CENTRAL),
                            ('R_Hip', LeftRightType.RIGHT),
                            ('R_Knee', LeftRightType.RIGHT),
                            ('R_Ankle', LeftRightType.RIGHT),
                            ('L_Hip', LeftRightType.LEFT),
                            ('L_Knee', LeftRightType.LEFT),
                            ('L_Ankle', LeftRightType.LEFT),
                            ('Torso', LeftRightType.CENTRAL),
                            ('Neck', LeftRightType.CENTRAL),
                            ('Nose', LeftRightType.CENTRAL),
                            ('Head', LeftRightType.CENTRAL),
                            ('L_Shoulder', LeftRightType.LEFT),
                            ('L_Elbow', LeftRightType.LEFT),
                            ('L_Wrist', LeftRightType.LEFT),
                            ('R_Shoulder', LeftRightType.RIGHT),
                            ('R_Elbow', LeftRightType.RIGHT),
                            ('R_Wrist', LeftRightType.RIGHT),
                            ('Thorax', LeftRightType.CENTRAL)],
            offset_keypoint_names=['Pelvis'],
            scale_keypoint_name_pairs=[(['Pelvis'], ['Torso']),
                                       (['Torso'], ['Neck'])],
            segment_name_pairs=[ (['Pelvis'], ['Torso']), (['Torso'], ['Neck']), 
                                (['Neck'], ['Nose']), (['Nose'], ['Head']), 
                                (['Neck'], ['L_Shoulder']), (['L_Shoulder'], ['L_Elbow']), 
                                (['L_Elbow'], ['L_Wrist']), (['Neck'], ['R_Shoulder']), 
                                (['R_Shoulder'], ['R_Elbow']), (['R_Elbow'], ['R_Wrist']), 
                                (['Pelvis'], ['R_Hip']), (['R_Hip'], ['R_Knee']), 
                                (['R_Knee'], ['R_Ankle']), (['Pelvis'], ['L_Hip']), 
                                (['L_Hip'], ['L_Knee']), (['L_Knee'], ['L_Ankle'])],
            head_keypoint_name=['Head'],
            neck_keypoint_name=['Neck'],
            left_shoulder_keypoint_name=['L_Shoulder'],
            right_shoulder_keypoint_name=['R_Shoulder'],
            left_elbow_keypoint_name=['L_Elbow'],
            right_elbow_keypoint_name=['R_Elbow'],
            left_wrist_keypoint_name=['L_Wrist'],
            right_wrist_keypoint_name=['R_Wrist'],
            spine_keypoint_name=['Torso'],
            pelvis_keypoint_name=['Pelvis'],
            left_hip_keypoint_name=['L_Hip'],
            right_hip_keypoint_name=['R_Hip'],
            left_knee_keypoint_name=['L_Knee'],
            right_knee_keypoint_name=['R_Knee'],
            left_ankle_keypoint_name=['L_Ankle'],
            right_ankle_keypoint_name=['R_Ankle'])


class ExtractedH36m17KeypointProfile3D(KeypointProfile3D):
    """Legacy Human3.6M 3D 17-keypoint profile."""

    def __init__(self):
        """Initializer."""
        super(ExtractedH36m17KeypointProfile3D, self).__init__(
            name='EXTRACTED_3DH36M17',          
            keypoint_names=[('Pelvis', LeftRightType.CENTRAL),
                            ('R_Hip', LeftRightType.RIGHT),
                            ('R_Knee', LeftRightType.RIGHT),
                            ('R_Ankle', LeftRightType.RIGHT),
    ...
    def __init__(self):
        ...
        super(Std13KeypointProfile2D, self).__init__(
            ...
            compatible_keypoint_name_dict={
                ...
                'COCO_17BODY_2D': [
                    'NOSE_TIP', 'LEFT_SHOULDER', 'RIGHT_SHOULDER', 'LEFT_ELBOW', 'RIGHT_ELBOW', 'LEFT_WRIST',
...
class Coco17KeypointProfile2D(KeypointProfile2D):
    """COCO 2D 17-keypoint profile.

    """
    def __init__(self):
        """Initializer."""
        super(Coco17KeypointProfile2D, self).__init__(
            name='COCO_17BODY_2D',
            keypoint_names=[('NOSE_TIP', LeftRightType.CENTRAL),    # 1
                            ('LEFT_EYE', LeftRightType.LEFT),       # 2
                            ('RIGHT_EYE', LeftRightType.RIGHT),     # 3
                            ('LEFT_EAR', LeftRightType.LEFT),       # 4
                            ('RIGHT_EAR', LeftRightType.RIGHT),     # 5
                            ('LEFT_SHOULDER', LeftRightType.LEFT),  # 6
                            ('RIGHT_SHOULDER', LeftRightType.RIGHT),# 7
                            ('LEFT_ELBOW', LeftRightType.LEFT),     # 8
                            ('RIGHT_ELBOW', LeftRightType.RIGHT),   # 9
                            ('LEFT_WRIST', LeftRightType.LEFT),     # 10
                            ('RIGHT_WRIST', LeftRightType.RIGHT),   # 11
                            ('LEFT_HIP', LeftRightType.LEFT),       # 12
                            ('RIGHT_HIP', LeftRightType.RIGHT),     # 13
                            ('LEFT_KNEE', LeftRightType.LEFT),      # 14
                            ('RIGHT_KNEE', LeftRightType.RIGHT),    # 15
                            ('LEFT_ANKLE', LeftRightType.LEFT),     # 16
                            ('RIGHT_ANKLE', LeftRightType.RIGHT)],  # 17
            offset_keypoint_names=['LEFT_HIP', 'RIGHT_HIP'],
            scale_keypoint_name_pairs=[(['LEFT_SHOULDER'], ['RIGHT_SHOULDER']), 
                                       (['LEFT_SHOULDER'], ['LEFT_HIP']),
                                       (['LEFT_SHOULDER'], ['RIGHT_HIP']),
                                       ...
                                       (['LEFT_HIP'], ['RIGHT_HIP'])],
            segment_name_pairs=[
                                (['RIGHT_EAR'], ['RIGHT_SHOULDER']),    # (5, 7)
                                (['LEFT_EAR'], ['LEFT_SHOULDER']),      # (4, 6)
                                (['RIGHT_EYE'], ['RIGHT_EAR']),         # (3, 5)
                                (['LEFT_EYE'], ['LEFT_EAR']),           # (2, 4)
                                (['NOSE_TIP'], ['RIGHT_EYE']),          # (1, 3)
                                (['NOSE_TIP'], ['LEFT_EYE']),           # (1, 2)
                                (['LEFT_EYE'], ['RIGHT_EYE']),          # (2, 3)
                                (['NOSE_TIP'], ['LEFT_SHOULDER']),      # (1, 6)
                                (['NOSE_TIP'], ['RIGHT_SHOULDER']),     # (1, 7)
                                (['LEFT_SHOULDER'], ['RIGHT_SHOULDER']),# (6, 7)
                                (['LEFT_SHOULDER'], ['LEFT_ELBOW']),    # (6, 8)
                                (['RIGHT_SHOULDER'], ['RIGHT_ELBOW']),  # (7, 9)
                                (['LEFT_ELBOW'], ['LEFT_WRIST']),   # (8, 10)
                                (['RIGHT_ELBOW'], ['RIGHT_WRIST']), # (9, 11)
                                (['LEFT_SHOULDER'], ['LEFT_HIP']),  # (6, 12)
                                (['RIGHT_SHOULDER'], ['RIGHT_HIP']),# (7, 13)
                                (['LEFT_HIP'], ['RIGHT_HIP']),      # (12, 13)
                                (['LEFT_HIP'], ['LEFT_KNEE']),      # (12, 14)
                                (['RIGHT_HIP'], ['RIGHT_KNEE']),    # (13, 15)
                                (['LEFT_KNEE'], ['LEFT_ANKLE']),    # (14, 16)
                                (['RIGHT_KNEE'], ['RIGHT_ANKLE'])], # (15, 17)
            compatible_keypoint_name_dict={
                'EXTRACTED_3DH36M17': [
                    'Nose', 'Head', 'Head', 'Neck', 'Neck', 'L_Shoulder', 'R_Shoulder', 'L_Elbow', 'R_Elbow', 'L_Wrist',
                    'R_Wrist', 'L_Hip', 'R_Hip', 'L_Knee', 'R_Knee', 'L_Ankle', 'R_Ankle'
                ]                                
            },        
            head_keypoint_name=['NOSE_TIP'],
...
class UpperBody9KeypointProfile2D(KeypointProfile2D):
    """Upper Body 2D 9-keypoint profile."""

    def __init__(self):
        """Initializer."""
        super(UpperBody9KeypointProfile2D, self).__init__(
            name='2DSTD13',
            keypoint_names=[('NOSE_TIP', LeftRightType.CENTRAL),
                            ('LEFT_SHOULDER', LeftRightType.LEFT),
                            ('RIGHT_SHOULDER', LeftRightType.RIGHT),
...
def create_keypoint_profile(keypoint_profile_name):
    ...
    """
    if keypoint_profile_name == 'EXTRACTED_3DH36M18':
        return ExtractedH36m18KeypointProfile3D()
    if keypoint_profile_name == 'EXTRACTED_3DH36M17':
        return ExtractedH36m17KeypointProfile3D()    
    if keypoint_profile_name == 'LEGACY_2DCOCO13':
        return LegacyCoco13KeypointProfile2D()
    if keypoint_profile_name == 'COCO_17BODY_2D':
        return Coco17KeypointProfile2D()    
    if keypoint_profile_name == 'UPPERBODY_9_2D':
        return UpperBody9KeypointProfile2D()    
    
    raise ValueError('Unsupported keypoint profile name: `%s`.' %
                     str(keypoint_profile_name))

def coco17_to_coco13(keypoint2d_17, coco_profile="LEGACY_2DCOCO13", conf=0.3, mask_type="RIGHT"):
    """ keypoint2d_17: input coco 17body joints shape [17, 3]--> [x, y, score]

        return:  the normalized coco 13 joints used in training the pose embedding model. shape [13, 3]
    """
    coco17 = create_keypoint_profile("COCO_17BODY_2D")
    coco13 = create_keypoint_profile(coco_profile)
    input_keypoint_names = coco17.keypoint_names
    output_keypoint_names = coco13.compatible_keypoint_name_dict[coco17.name]
    ...
    output_keypoints = keypoint2d_17[input_to_output_indices, ...]

    normalized_keypoints, offset_points, scale_distances = coco13.normalize(output_keypoints[:,:2])

    # mask out low confidence keypoints
    mask = output_keypoints[:,-1] > conf

    mask_out = []
    if mask_type != "NONE":
        if "UPPER" in mask_type:
            mask_out = mask_out + coco13.get_standard_part_index("LEFT_ANKLE") + \
                                coco13.get_standard_part_index("RIGHT_ANKLE")  + \
                                coco13.get_standard_part_index("LEFT_KNEE")    + \
                                coco13.get_standard_part_index("RIGHT_KNEE") 
            if "WRIST" in mask_type:
                if "LEFT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("LEFT_WRIST")
                if "RIGHT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("RIGHT_WRIST")
            if "ELBOW" in mask_type:
                if "LEFT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("LEFT_ELBOW")
                if "RIGHT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("RIGHT_ELBOW") 
        else:
            if "WRIST" in mask_type:
                if "LEFT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("LEFT_WRIST")
            ...
            if "ELBOW" in mask_type:
                ...
                if "RIGHT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("RIGHT_ELBOW") 
            if "ANKLE" in mask_type:
                if "LEFT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("LEFT_ANKLE")
                if "RIGHT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("RIGHT_ANKLE")     
            if "KNEE" in mask_type:
                if "LEFT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("LEFT_KNEE")
                if "RIGHT" in mask_type:
                    mask_out = mask_out + coco13.get_standard_part_index("RIGHT_KNEE")                                          
        mask[mask_out] = False
    # normalized_keypoints[mask, -1] = 1
    normalized_keypoints[~mask, :] = 0
    
    # normalized_keypoints = torch.cat([normalized_keypoints, output_keypoints[:, -1].unsqueeze(-1)], dim=-1)
    coco13_dict = {
        constants.KEY_OFFSET_POINTS_2D: offset_points,
        constants.KEY_SCALE_DISTANCES_2D: scale_distances, #.squeeze(0),
        constants.KEY_PREPROCESSED_KEYPOINTS_2D: normalized_keypoints, #.squeeze(0),
        constants.KEY_PREPROCESSED_KEYPOINT_MASKS_2D: mask #.squeeze(0)
    }
    features = torch.cat([normalized_keypoints, mask.unsqueeze(-1)], dim=-1)
    features = data_utils.flatten_last_dims(features, num_last_dims=2)
    coco13_dict.update(dict(model_inputs=features.unsqueeze(0))) 
    return coco13_dict


def coco17_to_upperbody9(keypoint2d_17, conf=0.3):
    """ keypoint2d_17: input coco 17body joints shape [17, 3]--> [x, y, score]

        return:  the normalized upperbody 9 joints used in training the pose embedding model. shape [9, 3]
    """
    coco17 = create_keypoint_profile("COCO_17BODY_2D")
    upperbody9 = create_keypoint_profile("UPPERBODY_9_2D")
    input_keypoint_names = coco17.keypoint_names
    output_keypoint_names = upperbody9.compatible_keypoint_name_dict[coco17.name]
    ...
    normalized_keypoints, offset_points, scale_distances = upperbody9.normalize(output_keypoints[:,:2])
    ...
    features = torch.cat([normalized_keypoints, mask.unsqueeze(-1)], dim=-1)
    features = data_utils.flatten_last_dims(features, num_last_dims=2)
    upperbody9_dict.update(dict(model_inputs=features.unsqueeze(0))) 
    return upperbody9_dict