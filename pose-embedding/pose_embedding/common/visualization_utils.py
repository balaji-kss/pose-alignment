...
def get_points(points, indices, keepdims=True, raise_error_if_empty=True):
    ...
    points = np.take(points_numpy, indices=indices, axis=-2)
...
def draw_poses_2d(keypoints_2d, keypoint_profile_2d, num_cols, subfigure_size=(1.5, 1.5), **kwargs):
    """Draws 2D poses.
    ...
    Returns:
      fig: A Matplotlib figure object with the drawn poses.
    """
    num_rows = (keypoints_2d.shape[0] + num_cols - 1) // num_cols
    subfigure_height, subfigure_width = subfigure_size
    fig = plt.figure(figsize=(num_cols * subfigure_width, num_rows * subfigure_height))
    fig_grid_spec = gridspec.GridSpec(num_rows, num_cols)
    plt.axis('off')

    for i, keypoints in enumerate(keypoints_2d):
        ax = plt.subplot(fig_grid_spec[i])
        # Here draw_pose_2d is your existing function that takes ax, keypoints, keypoint_profile_2d, and any other keyword arguments.
        draw_pose_2d(ax, keypoints, keypoint_profile_2d, **kwargs)

    fig.canvas.draw()

    return fig  # Returning the Matplotlib figure object


def plot_to_tensorboard(writer, fig, step, tag="2d_pose"):
    """
    ...
    Args:
        ...
        step: The current step or epoch in training
        tag: Tag to associate with the plot
    """
    ...
    img_arr = np.transpose(img_arr, (2, 0, 1))  # Convert from HWC to CHW
    
    # Log the image to tensorboard
    writer.add_image(tag, img_arr, step)
